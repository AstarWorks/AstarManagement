...
const visibleChildren = computed(() => {
  ...
  return props.item.children.filter(child => {
    ...
    if (child.requiredPermissions && child.requiredPermissions.length > 0) {
      const hasPermission = child.requireAny
        ? child.requiredPermissions.some(p => authStore.hasPermission(p))
        : child.requiredPermissions.every(p => authStore.hasPermission(p))
    ...
    if (child.requiredRoles && child.requiredRoles.length > 0) {
      const hasRole = child.requireAny
        ? child.requiredRoles.some(r => authStore.hasRole(r))
        : child.requiredRoles.every(r => authStore.hasRole(r))
