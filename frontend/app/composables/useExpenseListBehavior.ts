...
export function useExpenseListBehavior(
  ...
  emit: {
    (event: 'delete' | 'edit' | 'duplicate' | 'view', expense: IExpense): void
    (event: 'retry' | 'refresh'): void
    (event: 'scroll', data: { scrollTop: number; scrollLeft: number }): void
    (event: 'select', data: { expenseId: string; selected: boolean }): void
    (event: 'visibleRangeChange', data: { startIndex: number; endIndex: number }): void
  }
