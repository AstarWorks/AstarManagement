...
const statusConfig = computed(() => {
  const configs: Record<CaseStatus, { order: number; label: string; color: string }> = {
    ...
    negotiation: { order: 4, label: t('matter.status.negotiation'), color: 'bg-orange-500' },
    mediation: { order: 5, label: t('matter.status.mediation'), color: 'bg-purple-500' },
    court: { order: 6, label: t('matter.status.court'), color: 'bg-red-500' },
    resolved: { order: 7, label: t('matter.status.resolved'), color: 'bg-gray-500' }
  }
