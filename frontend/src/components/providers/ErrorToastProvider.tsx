...
import { getContextualErrorMessage, ErrorAction, ErrorType } from '@/services/error/error.handler'
...
export function ErrorToastProvider({ children }: ErrorToastProviderProps) {
  ...
  useEffect(() => {
    ...
    const contextualMessage = getContextualErrorMessage(error, 'general')
        ...
        case ErrorAction.CONTACT_SUPPORT:
          return {
            ...
            onClick: () => {
              ...
              console.log('Contact support requested for error:', error.correlationId)
            }
          }
        default:
          return {
  ...
  useEffect(() => {
    const unsubscribe = useKanbanStore.subscribe(
      (state) => state.matters,
      (matters, previousMatters) => {
        // Detect successful operations based on matter changes
        if (previousMatters.length < matters.length) {
          // New matter added
