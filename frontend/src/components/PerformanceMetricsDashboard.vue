  ...
  <div class="performance-dashboard" :class="{ 'minimized': isMinimized }">
    ...
    <div v-if="!isMinimized" class="dashboard-content">
      ...
      <div class="quick-stats">
        <div class="stat-item" :class="getPerformanceClass(metrics.performanceSummary.value.averageQueryTime)">
          <div class="stat-value">{{ metrics.performanceSummary.value.averageQueryTime.toFixed(0) }}ms</div>
          <div class="stat-label">Avg Response</div>
        </div>
        
        <div class="stat-item">
          <div class="stat-value">{{ metrics.performanceSummary.value.cacheHitRate.toFixed(0) }}%</div>
        ...
        <div class="stat-item" :class="{ 'has-errors': metrics.performanceSummary.value.failedQueries > 0 }">
          <div class="stat-value">{{ metrics.performanceSummary.value.failedQueries }}</div>
      ...
      <div class="performance-score">
        ...
        <div class="score-details">
          <div class="score-item">
            <span>P95: {{ metrics.performanceSummary.value.p95QueryTime.toFixed(0) }}ms</span>
            <span :class="getPerformanceClass(metrics.performanceSummary.value.p95QueryTime)">
              {{ getPerformanceStatus(metrics.performanceSummary.value.p95QueryTime) }}
            </span>
          </div>
          <div class="score-item">
    ...
    <div v-else class="minimized-content">
      <div class="mini-stat" :class="getPerformanceClass(metrics.performanceSummary.value.averageQueryTime)">
        <Icon name="activity" :size="14" />
        <span>{{ metrics.performanceSummary.value.averageQueryTime.toFixed(0) }}ms</span>
      </div>
      <div class="mini-stat">
        <Icon name="database" :size="14" />
        <span>{{ metrics.performanceSummary.value.cacheHitRate.toFixed(0) }}%</span>
      </div>
    </div>
  </div>
</template>
...
const Icon = defineComponent({
  ...
  setup(props) {
    ...
    return () => h(icons[props.name as keyof typeof icons] || Activity, { size: props.size })
  }
})
