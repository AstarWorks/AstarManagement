...
const props = withDefaults(defineProps<Props>(), {
  ...
  columns: () => [
    { id: 'intake', title: 'Intake', status: 'INTAKE' as MatterStatus, color: 'gray', order: 0, visible: true, acceptsDrop: true, currentItemCount: 0 },
    { id: 'initial-review', title: 'Initial Review', status: 'INITIAL_REVIEW' as MatterStatus, color: 'blue', order: 1, visible: true, acceptsDrop: true, currentItemCount: 0 },
    { id: 'in-progress', title: 'In Progress', status: 'IN_PROGRESS' as MatterStatus, color: 'yellow', order: 2, visible: true, acceptsDrop: true, currentItemCount: 0 },
    { id: 'review', title: 'Review', status: 'REVIEW' as MatterStatus, color: 'orange', order: 3, visible: true, acceptsDrop: true, currentItemCount: 0 },
    { id: 'waiting-client', title: 'Waiting Client', status: 'WAITING_CLIENT' as MatterStatus, color: 'purple', order: 4, visible: true, acceptsDrop: true, currentItemCount: 0 },
    { id: 'ready-filing', title: 'Ready Filing', status: 'READY_FILING' as MatterStatus, color: 'green', order: 5, visible: true, acceptsDrop: true, currentItemCount: 0 },
    { id: 'closed', title: 'Closed', status: 'CLOSED' as MatterStatus, color: 'gray', order: 6, visible: true, acceptsDrop: true, currentItemCount: 0 }
  ]
})
...
// Computed properties
const matters = computed<MatterCard[]>(() => {
  const rawMatters = mattersData.value?.data || []
  return rawMatters.map((matter, index) => ({
    ...matter,
    position: index,
    // Ensure assignedLawyer is in the correct format
    assignedLawyer: typeof matter.assignedLawyer === 'string' 
      ? { id: matter.assignedLawyer, name: matter.assignedLawyer, initials: matter.assignedLawyer.substring(0, 2).toUpperCase() }
      : matter.assignedLawyer as { id: string; name: string; avatar?: string; initials?: string } | undefined
  } as MatterCard))
})

const getColumnMatters = (status: MatterStatus): MatterCard[] => {
...
const executeBatchStatusChange = async (targetStatus: MatterStatus) => {
  ...
  try {
    ...
    const operations = selectedMattersList.map((matter, index) => ({
      ...
      fromIndex: (matter as MatterCard).position || 0,
      toIndex: index, // Sequential placement
      matter: matter as MatterCard
    }))
...
const handleBatchArchive = async () => {
  await executeBatchStatusChange('CLOSED' as MatterStatus)
}
...
const getStatusTransitions = (status: MatterStatus): MatterStatus[] => {
  const transitions: Record<MatterStatus, MatterStatus[]> = {
    ...
    'IN_PROGRESS': ['REVIEW', 'WAITING_CLIENT', 'CLOSED'],
    'REVIEW': ['IN_PROGRESS', 'READY_FILING', 'WAITING_CLIENT', 'CLOSED'],
    'WAITING_CLIENT': ['IN_PROGRESS', 'REVIEW', 'CLOSED'],
    'READY_FILING': ['CLOSED', 'REVIEW'],
    'CLOSED': ['INTAKE', 'INITIAL_REVIEW']
  }
  ...
  if (event.ctrlKey || event.metaKey) {
    switch (event.key) {
      case 'a':
        ...
        selectAll(matters.value as MatterCard[])
        break
      case 'Escape':
        clearSelection()
        break
