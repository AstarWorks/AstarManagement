          ...
          <div class="space-y-2">
            <div class="flex items-center space-x-2">
              <Checkbox
                id="show-closed"
                :checked="showClosed"
                @update:checked="(checked: boolean) => updateShowClosed(checked)"
              />
...
const selectedLawyers = computed(() => filterState.value.filters?.filter(f => f.field === 'lawyer').map(f => f.value) || [])
const selectedPriorities = computed(() => filterState.value.filters?.filter(f => f.field === 'priority').map(f => f.value) || [])
const selectedStatuses = computed(() => filterState.value.filters?.filter(f => f.field === 'status').map(f => f.value) || [])
const showClosed = computed(() => filterState.value.filters?.some(f => f.field === 'showClosed' && f.value) || false)

// Undo/Redo functionality (placeholder implementation)
...
const updateShowClosed = (checked: boolean) => {
  ...
  const otherFilters = currentFilters.filter(f => f.field !== 'showClosed')
  
  const newFilters = checked 
    ? [...otherFilters, { field: 'showClosed', operator: 'equals' as const, value: true }]
    : otherFilters
...
const filterStats = computed(() => ({
  ...
  activeFiltersCount: [
    currentFilters.value.quickSearch ? 1 : 0,
    selectedLawyers.value.length > 0 ? 1 : 0,
    selectedPriorities.value.length > 0 ? 1 : 0,
    selectedStatuses.value.length > 0 ? 1 : 0,
    showClosed.value ? 1 : 0
