...
const handleUpdate = async (matter: Matter, updates: Partial<Matter>) => {
  try {
    ...
    const updated = await updateMutation.mutateAsync({
      id: matter.id,
      data: updates as UpdateMatterInput
    })
    const latency = Date.now() - startTime
...
const mattersByStatus = computed(() => {
  const groups = {
    INTAKE: matters.value.filter(m => m.status === 'INTAKE'),
    IN_PROGRESS: matters.value.filter(m => m.status === 'IN_PROGRESS'), 
    REVIEW: matters.value.filter(m => m.status === 'REVIEW'),
    CLOSED: matters.value.filter(m => m.status === 'CLOSED')
  }
  return groups
})
