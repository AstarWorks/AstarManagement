  ...
  configure({
    ...
    generateMessage: (ctx) => {
      const messages: Record<string, string> = {
        ...
        min: `${ctx.field} must be at least ${Array.isArray(ctx.rule?.params) ? ctx.rule.params[0] : ''} characters`,
        max: `${ctx.field} must not exceed ${Array.isArray(ctx.rule?.params) ? ctx.rule.params[0] : ''} characters`,
