...
import { computed, inject, provide, readonly, ref, type Ref, type ComputedRef } from 'vue'
...
export function useField<TValue = any>(
  ...
  const field = useVeeField<TValue>(name, undefined)
...
export function useFieldContext(context: FormFieldContext) {
  ...
  const reactiveContext = ref(context)
  
  // Provide field context to children
  provide('formField', reactiveContext)
  
  return {
    context: readonly(reactiveContext)
  }
}
...
export function useFieldContext_access() {
  const context = inject<Ref<FormFieldContext> | null>('formField', null)
  
  return {
    context: context ? readonly(context) : readonly(ref(null)),
    hasContext: computed(() => !!context?.value)
  }
}
...
export function useFieldWithTriggers<TValue = any>(
  ...
  const field = useField<TValue>(name, options)
