...
export const kanbanMoveMatterSchema = z.object({
  ...
  newStatus: z.enum(['INTAKE', 'INITIAL_REVIEW', 'IN_PROGRESS', 'REVIEW', 'WAITING_CLIENT', 'READY_FILING', 'CLOSED']),
...
export function useKanbanMutations() {
  ...
  const statusChangeMutation = useMutation({
    ...
    onMutate: async (input: KanbanMoveMatterInput) => {
      ...
      if (dragOp) {
        dragOp.currentStatus = input.newStatus as MatterStatus
    ...
    onMutate: async (input) => {
      ...
      if (previousMatters) {
        ...
        if (matterIndex !== -1) {
          ...
          // Update position if it exists on the matter
          if ('position' in matter) {
            (matter as any).position = input.newPosition
          }
        ...
        operations.forEach(op => {
          ...
          if (index !== -1) {
            updatedMatters[index] = {
              ...updatedMatters[index],
              status: op.newStatus as MatterStatus,
              updatedAt: new Date().toISOString()
            }
            // Update position if it exists on the matter
            if ('position' in updatedMatters[index]) {
              (updatedMatters[index] as any).position = op.newPosition
            }
          }
        })
    ...
    if (event.added) {
      ...
      try {
        ...
        await kanbanMutations.updateMatterStatus({
          matterId: matter.id,
          newStatus: targetStatus as 'INTAKE' | 'INITIAL_REVIEW' | 'IN_PROGRESS' | 'REVIEW' | 'WAITING_CLIENT' | 'READY_FILING' | 'CLOSED',
          newPosition,
