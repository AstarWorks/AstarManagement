<template>
  <div class="container mx-auto p-6">
    ...
    <div class="mb-6 p-4 bg-blue-50 rounded">
      ...
      <p class="text-sm"><strong>Data Count:</strong> {{ data?.data?.length || 0 }}</p>
    ...
    <div v-if="!isPending && data" class="space-y-4">
      <h2 class="text-lg font-semibold">Matters ({{ data.data.length }})</h2>
      
      <div v-for="matter in data.data" :key="matter.id" class="p-4 border rounded hover:bg-gray-50">
        ...
        >
          {{ updateMutation.isPending ? 'Updating...' : 'Mark Completed' }}
        </button>
      </div>
    </div>
...
const updateStatus = (id: string, status: string) => {
  updateMutation.mutate({ matterId: id, newStatus: status as any })
}
