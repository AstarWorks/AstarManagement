...
import type { Matter, MatterCard, MatterStatus, MatterPriority } from '~/types/matter'
import type { DragDropEvent } from '~/composables/useKanbanDragDrop'
...
export function createMockMatterCard(overrides: Partial<MatterCard> = {}): MatterCard {
  const matter = createMockMatter(overrides)
...
export function createMockMattersByStatus(): Record<MatterStatus, MatterCard[]> {
  return {
    ...
    investigation: [
      createMockMatterCard({ id: 'matter-3', title: 'Evidence Review', status: 'investigation', position: 0 }),
      createMockMatterCard({ id: 'matter-4', title: 'Fact Finding', status: 'investigation', position: 1 })
    ],
    negotiation: [
      createMockMatterCard({ id: 'matter-5', title: 'Settlement Talks', status: 'negotiation', position: 0 })
    ],
    litigation: [
      createMockMatterCard({ id: 'matter-6', title: 'Court Filing', status: 'litigation', position: 0 })
    ],
    settlement: [],
    collection: [],
    closed: [
...
export function createMockColumnsWithCounts() {
  return [
    ...
    { id: 'investigation', title: 'Investigation', titleJa: '調査', count: 2 },
    { id: 'negotiation', title: 'Negotiation', titleJa: '交渉', count: 1 },
    { id: 'litigation', title: 'Litigation', titleJa: '訴訟', count: 1 },
...
export function mountKanbanComponent(
  ...
  Object.entries({ ...storeMocks, ...mockStores }).forEach(([name, mock]) => {
    vi.mocked(vi.doMock)(`~/stores/${name.replace('use', '').replace('Store', '').toLowerCase()}`, () => ({ [name]: mock }))
  })
...
export function assertMatterCardRendered(
  ...
  const title = showJapanese ? matter.titleJa : matter.title
  const clientName = showJapanese ? matter.clientNameJa : matter.clientName
