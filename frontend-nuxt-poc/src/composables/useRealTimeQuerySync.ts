...
export function useRealTimeQuerySync(config: Partial<SyncConfiguration> = {}) {
  ...
  const isConnected = computed(() => realTimeStore.isOnline.value)
  const isSyncing = computed(() => realTimeStore.isSyncing.value)
  const hasUnresolvedConflicts = computed(() => realTimeStore.hasConflicts.value)
