...
export function useKanbanDragDropMutations() {
  ...
  const calculatePosition = (input: PositionCalculationInput): number => {
    ...
    if (targetIndex >= existingMatters.length) {
      return existingMatters.length > 0 
        ? existingMatters[existingMatters.length - 1].position + 1000
        : 1000
    }
    ...
    if (targetIndex === 0) {
      return existingMatters.length > 0
        ? Math.max(existingMatters[0].position - 1000, 0)
        : 1000
    }
    ...
    return Math.floor((prevMatter.position + nextMatter.position) / 2)
  }
      ...
      onSuccess: (data, input) => {
        ...
        queryClient.setQueryData(queryKeys.lists(), (old: any) => {
          ...
          const updatedData = old.data.map((matter: MatterCard) => {
            ...
            if (serverMatter && matter.batchId === input.timestamp) {
              return { ...serverMatter, isOptimistic: false, batchId: undefined }
            }
            return matter
          })
  ...
  return {
    ...
    // Loading states
    isMoving: computed(() => moveMatterMutation.isPending.value),
    isBatchMoving: computed(() => batchMoveMutation.isPending.value),
    isDragOperationPending: computed(() => 
      moveMatterMutation.isPending.value || batchMoveMutation.isPending.value
    ),
    
    // Error states
    moveError: computed(() => moveMatterMutation.error.value),
    batchMoveError: computed(() => batchMoveMutation.error.value),
