<template>
  <div :class="cn(fieldWrapperClasses, props.class)">
    ...
    >
      <div
        v-if="field.hasError.value && props.showError"
        :id="`${fieldId}-error`"
    ...
    <!-- Help Text -->
    <div
      v-if="helpText && !field.hasError && !warning"
      :id="`${fieldId}-help`"
...
/**
 * FormField slot props interface
 */
...
const describedByIds = computed(() => {
  ...
  if (field.hasError && props.showError) {
...
const labelClasses = computed(() => {
  ...
  const errorClasses = field.hasError
    ? 'text-destructive'
    : 'text-foreground'
