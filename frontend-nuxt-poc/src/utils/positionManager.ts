...
export function calculateInsertPosition(
  ...
  const sortedMatters = filteredMatters.sort((a, b) => a.position - b.position)
  ...
  if (targetIndex === 0) {
    const firstPosition = sortedMatters[0].position
  ...
  if (targetIndex >= sortedMatters.length) {
    const lastPosition = sortedMatters[sortedMatters.length - 1].position
  ...
  const middlePosition = Math.floor((prevMatter.position + nextMatter.position) / 2)
...
export function detectPositionConflicts(matters: MatterCard[]): PositionConflict[] {
  ...
  matters.forEach(matter => {
    if (!positionMap.has(matter.position)) {
      positionMap.set(matter.position, [])
    }
    positionMap.get(matter.position)!.push(matter)
  })
...
export function calculateBatchPositions(
  ...
  Object.entries(operationsByStatus).forEach(([status, statusOps]) => {
    ...
    sortedOps.forEach((op, opIndex) => {
      ...
      statusMatters.sort((a, b) => a.position - b.position)
    })
  })
...
export function optimizePositions(matters: MatterCard[]): MatterCard[] {
  const sortedMatters = matters.sort((a, b) => a.position - b.position)
...
export function needsNormalization(matters: MatterCard[]): boolean {
  ...
  for (let i = 0; i < sortedMatters.length - 1; i++) {
    const gap = sortedMatters[i + 1].position - sortedMatters[i].position
...
export function getPositionStatistics(matters: MatterCard[]) {
  ...
  const gaps = []
  for (let i = 0; i < sortedMatters.length - 1; i++) {
    gaps.push(sortedMatters[i + 1].position - sortedMatters[i].position)
  }
  ...
  return {
    ...
    minPosition: sortedMatters[0].position,
    maxPosition: sortedMatters[sortedMatters.length - 1].position,
    averageGap,
