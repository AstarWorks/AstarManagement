...
```json
{
  ...
  "data": {
    ...
    "urls": {
      "original": "https://storage.supabase.co/v1/object/authenticated/attachments/tenant-123/temp/2024-01/att-550e8400.jpg",
      "thumbnail": "https://storage.supabase.co/v1/object/authenticated/attachments/tenant-123/temp/2024-01/att-550e8400_thumb.webp"
    },
...
serve(async (req) => {
  ...
  // サムネイル生成（200x200）
  const thumbnail = await resizeImage(original, 200, 200)
  
  // WebP形式で保存
  const thumbnailPath = fileName.replace(/\.[^.]+$/, '_thumb.webp')
...
fun cleanupExpiredFiles() {
    ...
    expiredFiles.forEach { attachment ->
        ...
        supabaseClient.storage
            .from("attachments")
            .remove(listOf(attachment.storagePath))
