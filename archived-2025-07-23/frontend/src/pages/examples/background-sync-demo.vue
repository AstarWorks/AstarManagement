      ...
      <Card class="mb-6">
        ...
        <CardContent>
          <RadioGroup 
            ...
            @update:model-value="(value: string) => setSyncMode(value as SyncMode)"
      ...
      <Card class="mb-6">
        ...
        <CardContent>
          <div class="space-y-4">
            ...
            <Alert v-if="batteryLevel !== null && batteryLevel < 20" variant="destructive">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div v-else class="space-y-2">
            ...
            <p v-if="!matters || matters.length === 0" class="text-center py-4 text-muted-foreground">
...
const getQueryConfig = computed(() => {
  // Type guard to ensure we have a valid SyncMode
  const validModes: SyncMode[] = ['aggressive', 'balanced', 'conservative', 'offline', 'manual']
  const typedMode = validModes.includes(syncMode.value as SyncMode) ? syncMode.value as SyncMode : 'balanced'
  const config = getBackgroundSyncConfig('matters', typedMode)
  return {
    staleTime: config.staleTime,
    refetchInterval: config.baseInterval > 0 ? config.baseInterval : false as number | false,
...
const {
  ...
  staleTime: computed(() => getQueryConfig.value.staleTime),
  refetchInterval: computed(() => getQueryConfig.value.refetchInterval),
  refetchIntervalInBackground: computed(() => getQueryConfig.value.refetchIntervalInBackground),
  refetchOnWindowFocus: computed(() => getQueryConfig.value.refetchOnWindowFocus),
  refetchOnReconnect: computed(() => getQueryConfig.value.refetchOnReconnect),
  enabled: computed(() => getQueryConfig.value.enabled)
})
