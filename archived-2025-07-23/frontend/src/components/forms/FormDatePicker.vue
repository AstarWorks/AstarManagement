      ...
      <Popover v-model:open="isOpen">
        ...
        <PopoverContent class="w-auto p-0" align="start">
          <Calendar
            ...
            :mode="mode"
            :disabled="typeof disabledDates === 'function' ? false : disabledDates"
            :min-date="minDate"
            :max-date="maxDate"
...
// Computed property for range display
const rangeDisplayText = computed(() => {
  if (props.mode === 'range' && field.value.value && typeof field.value.value === 'object' && field.value.value !== null && 'start' in field.value.value && 'end' in field.value.value) {
    return formatDateRange(field.value.value as { start: string; end: string })
  }
  return ''
})
