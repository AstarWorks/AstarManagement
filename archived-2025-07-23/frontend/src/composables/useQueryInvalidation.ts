...
export function useQueryInvalidation() {
  ...
  const setupDefaultRules = () => {
    const defaultRules: InvalidationRule[] = [
      ...
      // Matter creation - invalidate lists and statistics
      {
        ...
        queryKeys: [queryKeys.lists(), queryKeys.matters.statistics(), queryKeys.statusCounts()],
        debounceMs: 300,
        cascade: false
      },
  ...
  const executeCascadeInvalidation = async (matterId: string, eventType: string, context: InvalidationContext) => {
    const cascadeQueries: (readonly unknown[])[] = []
    ...
    switch (eventType) {
      ...
      case 'matter_unassigned':
        ...
        if (context.event?.data?.assignedLawyer) {
          cascadeQueries.push(queryKeys.matters.assignedMatters(context.event.data.assignedLawyer))
        }
        if (context.event?.data?.previousAssignedLawyer) {
          cascadeQueries.push(queryKeys.matters.assignedMatters(context.event.data.previousAssignedLawyer))
        }
        break
      ...
      case 'matter_deleted':
        ...
        cascadeQueries.push(queryKeys.lists())
        cascadeQueries.push(queryKeys.matters.statistics())
        break
    }
   ...
   */
  const invalidateQueries = async (queryKeysToInvalidate: readonly (readonly unknown[])[], options?: { cascade?: boolean }) => {
    ...
    if (options?.cascade) {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.matters.statistics() })
    }
    
    metrics.value.totalInvalidations += queryKeysToInvalidate.length
    metrics.value.lastInvalidationTime = new Date()
  }
  ...
  const processBatchInvalidation = async (events: RealTimeEvent[]) => {
    ...
    try {
      ...
      for (const [eventType, groupEvents] of Object.entries(eventGroups)) {
        // Type guard to ensure groupEvents is an array
        if (!Array.isArray(groupEvents)) continue
        
        // For matter updates, we can batch invalidate
        if (eventType === 'matter_updated' && groupEvents.length > 1) {
          const matterIds = groupEvents
            .map(e => e.data?.id)
