...
export function useRealtimeSync() {
  ...
  try {
    ...
    console.warn('Query client not available yet, realtime sync will be initialized later')
    // Return a mock object
    return {
      connectionState: ref({ status: 'disconnected', reconnectAttempts: 0 }),
      connect: () => {},
      disconnect: () => {},
      send: () => false,
      getStats: () => ({}),
      latency: ref(null),
      messagesPerMinute: ref(0),
      lastReceivedMessage: ref(null),
      isConnected: ref(false)
    }
  }
  
  if (!queryClient) {
    console.warn('Query client is null, realtime sync will be initialized later')
    return {
      connectionState: ref({ status: 'disconnected', reconnectAttempts: 0 }),
      connect: () => {},
      disconnect: () => {},
