...
export function useKanbanRealTimeSync(options?: {
  ...
  const { subscribeToUpdates, send, isConnected } = useRealtimeSync()
  ...
  const autoResolveConflict = async (conflictId: string, conflict: ConflictInfo) => {
    ...
    try {
      ...
      if (localChanges.status === serverChanges.status) {
        // Since Matter doesn't have position, we skip position merging
        const mergedPosition = 0
  ...
  const startSync = () => {
    ...
    if (subscribeToUpdates) {
      subscribeToUpdates('matter_moved', handleMatterMoved as (event: any) => void)
      subscribeToUpdates('matter_batch_moved', handleBatchMoved as (event: any) => void)
      subscribeToUpdates('position_normalized', handlePositionNormalized as (event: any) => void)
