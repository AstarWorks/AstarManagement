...
export function useWebVitals() {
  ...
  const initializeWebVitals = async () => {
    ...
    try {
      ...
      onLCP(recordMetric)
      onINP(recordMetric) // INP replaced FID in web-vitals v4+
      onCLS(recordMetric)
  ...
  const performanceScore = computed(() => {
    ...
    if (metrics.value.inp !== null) {
      metricsCount++
      const rating = getRating('inp', metrics.value.inp)
      if (rating === 'needs-improvement') score -= 10
      else if (rating === 'poor') score -= 25
    }
 ...
 */
export async function useWebVitalsWithQueryMetrics() {
  const webVitals = useWebVitals()
  const { getGlobalQueryPerformanceMonitor } = await import('./useQueryPerformanceMonitor')
  ...
  const combinedPerformanceScore = computed(() => {
    ...
    // Calculate query score from performance metrics
    const queryScore = queryMetrics.performanceSummary.value?.averageQueryTime 
      ? Math.max(0, 100 - (queryMetrics.performanceSummary.value.averageQueryTime / 10))
      : 100
  ...
  const performanceRecommendations = computed(() => {
    ...
    if (webVitals.metrics.value.inp && webVitals.metrics.value.inp > 200) {
